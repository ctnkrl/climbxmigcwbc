...
class BodyServoIdMap:
    ...
    def body_can_id_map_init(self):
        ...
        # 头部关节映射 (29-30)
        head_ids = [1, 2]
        head_names = ["head_pitch", "head_yaw"]
        self.head_motor_nums = len(head_ids)

        # 合并所有映射
        all_ids = leg_ids + waist_ids + arm_ids + head_ids
        all_names = leg_names + waist_names + arm_names + head_names
