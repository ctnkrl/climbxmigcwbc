...
class XBOXController:
    ...
    def _load_config(self):
        try:
            config_path = os.path.join('.', 'config', 'control_tool.yaml')
    ...
    def xbox_flag_update(self):
        ...
        with self.data_mutex:
            ...
            elif self.map.l_trigger < -0.5 and self.map.a == 1:
                self.flag.fsm_state_command = 'gotoDH'
            # p -> gotoPBHC (Left trigger + B)
            elif self.map.l_trigger < -0.5 and self.map.b == 1:
                self.flag.fsm_state_command = 'gotoPBHC'
            # m -> gotoMLP (Left trigger + X)
            elif self.map.l_trigger < -0.5 and self.map.x == 1:
                self.flag.fsm_state_command = 'gotoMLP'
            # m+h combined style: LB + X -> MLPH
            elif self.map.lb == 1 and self.map.x == 1:
                self.flag.fsm_state_command = 'gotoMLPH'
            ...
            # gotoVELOCITY (Left trigger + select)
            elif self.map.l_trigger < -0.5 and self.map.select == 1:
                self.flag.fsm_state_command = 'gotoVELOCITY'
            # o -> gotoPBHCZERO (B button alone)
            elif self.map.b == 1:
                self.flag.fsm_state_command = 'gotoPBHCZERO'
            # g -> gotoDHZERO (A button alone)
            elif self.map.a == 1:
                self.flag.fsm_state_command = 'gotoDHZERO'
            # z -> gotoZERO
            elif self.map.x == 1:
