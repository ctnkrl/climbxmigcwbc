...
class FSMStateKNEELDOWN(FSMState):
    def __init__(self, robot_data: RobotData):
        ...
        config_path = os.path.join(current_dir, "config", "kneel_down.yaml")
        with open(config_path, "r") as f:
            ...
            self.warm_start_time = config.get("warm_start_time", 0.4)
            self.kps = config["kps"]
            self.kds = config["kds"]
    ...
    def inner_run(self,flag:FSMControlFlag):
        ...
        if time_index >= hold_start:
            if flag.fsm_state_command != "gotoKneelDown":
