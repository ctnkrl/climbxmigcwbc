...
class FSMStateSITRISE(FSMState):
    default_config_name = "sitrise.yaml"
            ...
            for prop in metadata_props:
                ...
                if prop.key in ("motion_length", "time_step_total"):
                    try:
                        ...
                        xlog.warning(f"[{self.__class__.__name__}] Invalid motion_length metadata value: {prop.value}")

            if self.motion_length is not None:
                try:
                    self.motion_length = int(self.motion_length)
                except (TypeError, ValueError):
                    xlog.warning(f"[{self.__class__.__name__}] Invalid motion_length config value: {self.motion_length}")
    ...
    def inner_run(self):
        ...
        # motion_phase: normalized position in the reference clip [0.0, 1.0].
        # Always clamped at 1.0 once the motion ends, regardless of hold_final_reference.
        _raw_idx = max(self.counter_step - self.warm_start_steps, 0)
        if self.motion_length is not None and self.motion_length > 1.0:
            motion_phase_val = float(min(_raw_idx, self.motion_length - 1)) / (self.motion_length - 1)
        else:
            motion_phase_val = 1.0
        motion_phase = np.array([motion_phase_val], dtype=np.float32)
    ...
    def _get_current_joint_pos_lab(self) -> np.ndarray:
        try:
            ...
            xlog.warning(f"[{self.__class__.__name__}] Failed to read current joint pose: {exc}")
    ...
    def _get_onnx_first_frame_lab(self) -> np.ndarray:
        try:
            ...
            xlog.warning(f"[{self.__class__.__name__}] Failed to read ONNX first frame: {exc}")
            return np.array(self.default_angles_lab, copy=True)
