...
from xmigcs.policy.navigate.agent import (
    agent_navigate_lab,
    agent_navigate_lab_12dof,
    agent_navigate_lab_sym,
    agent_navigate_lab_15dof,
    agent_navigate_lab_15dof_v2,
)
from xmigcs.utils.logging_utils import get_logger
logger = get_logger(__name__)

class FSMStateNAVIGATE(FSMState):
    def __init__(self, robot_data: RobotData):
        ...
        # self.agent = agent_navigate_lab()
        # self.agent = agent_navigate_lab_sym()
        # self.agent = agent_navigate_lab_12dof()
        # self.agent = agent_navigate_lab_15dof()
        self.agent = agent_navigate_lab_15dof_v2()
        config_path = os.path.join(current_dir, "config", "navigate_lab.yaml")

        with open(config_path, "r") as f:
            policy_config = yaml.safe_load(f)

        self.env_to_agent = self.agent.env_to_agent
    ...
    def on_enter(self):
        logger.info("[FSMStateNAVIGATE] enter")
        self.init_flag = True
        self.warm_start_pose = self.robot_data_.get_serial_joint_pos_desired()[self.env_to_agent]
        self.warm_start_time = self.robot_data_.time_now_

    def on_exit(self):
        logger.info("[FSMStateNAVIGATE] exit")
    ...
    def set_kp_kd(self):
        # Set kp/kd gains
        self.robot_data_.joint_kp_p_[self.env_to_agent] = self.kp[self.env_to_agent]
        self.robot_data_.joint_kd_p_[self.env_to_agent] = self.kd[self.env_to_agent]

    def write_to_robot_data(self, output_joint_pos):
        """将输出写入机器人数据"""
        # [6:]跳过前6维浮动基
        # NOTE: 只写入agent控制的 其余不写入
        self.robot_data_.q_d_[6:][self.env_to_agent] = output_joint_pos
        self.robot_data_.q_dot_d_[6:][self.env_to_agent] = 0.0
        self.robot_data_.tau_d_[6:][self.env_to_agent] = 0.0
        # 写入估计的基座线速度
    ...
    def get_observation_group(self) -> Dict[str, np.ndarray]:
        dof_pos = self.robot_data_.get_joint_pos()[self.env_to_agent]
        dof_vel = self.robot_data_.get_joint_vel()[self.env_to_agent]
        ...
        logger.debug("walk commands: %s", commands)
