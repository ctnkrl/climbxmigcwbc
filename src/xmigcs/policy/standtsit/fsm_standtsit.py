...
class FSMStateSTANDTSIT(FSMState):
    default_config_name = "standtsit.yaml"
    ...
    def inner_run(self):
        ...
        if self.motion_length is not None and self.motion_length > 1:
    ...
    def check_transition(self, *args, **kwargs):
        ...
        if action == "enter":
            #TODO: 判断是否允许切入 standtsit
            return result
        else :
            # 如果目标状态为停止，则允许退出 standtsit
            if target_state is not None and target_state.name in white_list.values():
                return result
            # 如果当前机器人未执行完成 standtsit 动作，则禁止退出 standtsit
            if not self.is_motion_end:
                current_time = time.perf_counter()
                if current_time - self._last_exit_blocked_log_time >= 5.0:
                    xlog.warning("[FSMStateSTANDTSIT] 当前机器人未执行完成 standtsit 动作，禁止退出 standtsit")
