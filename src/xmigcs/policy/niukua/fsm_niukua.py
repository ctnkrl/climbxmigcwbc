...
class FSMStateNIUKUA(FSMState):
    default_config_name = "niukua.yaml"
    ...
    def __init__(self, robot_data: RobotData, config_path: Optional[str] = None, variant_name: str = "default"):
        ...
        policy_dir = os.path.dirname(os.path.dirname(self.config_path))
        with open(self.config_path, "r") as f:
            config = yaml.load(f, Loader=yaml.FullLoader)
            self.onnx_path = config["onnx_path"]
            if not os.path.isabs(self.onnx_path):
                self.onnx_path = os.path.join(policy_dir, "model", self.onnx_path)
    ...
    def _play_remote_audio_YYW(self):
        ...
        command = "aplay ~/Documents/YYW.wav"
    ...
    def inner_run(self):
        ...
        if(self.counter_step < 2):
            init_ref_anchor_to_world_isaac = self.matrix_from_quat(self.yaw_quat(ref_anchor_ori_w))
            init_real_anchor_to_world_real = self.matrix_from_quat(self.yaw_quat(robot_quat))
            self.world_isaac_to_world_real = init_real_anchor_to_world_real @ init_ref_anchor_to_world_isaac.T
            self.counter_step += 1
            return

        robot_rot_mat = self.matrix_from_quat(robot_quat)
        motion_anchor_ori_b = robot_rot_mat.T @ self.world_isaac_to_world_real @ self.matrix_from_quat(ref_anchor_ori_w)
