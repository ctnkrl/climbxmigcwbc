...
class FSMStateKNEELDOWNNOARM(FSMState):
    def __init__(self, robot_data: RobotData):
        ...
        config_path = os.path.join(current_dir, "config", "kneeldownnoarm.yaml")
        with open(config_path, "r") as f:
            ...
            self.start_frame = int(config.get("start_frame", 0.0))
            self.fast_play_source_frames = int(config.get("fast_play_source_frames", 100))
            self.fast_play_output_steps = int(config.get("fast_play_output_steps", 50))
            ...
            self.model_ref_joint_pos = None
            self.model_ref_joint_vel = None
            # load policy
            ...
            if self.motion_length is not None and self.motion_length > 0:
                self.start_frame = min(self.start_frame, self.motion_length - 1)
            
            # # 根据YAML配置设置关节映射
            # self.mj2lab = np.array(config["mj2lab"], dtype=np.int32)
            
            self.joint_xml = FULL_LAB_JOINT_NAMES
            self.policy_joint_names = POLICY_OBS_JOINT_NAMES

            if self.joint_seq is None:
                self.joint_seq = list(self.joint_xml)

            unknown_joints = [joint for joint in self.joint_seq if joint not in self.joint_xml]
            if unknown_joints:
                raise ValueError(f"Unknown policy joints in metadata: {unknown_joints}")
            missing_policy_joints = [joint for joint in self.policy_joint_names if joint not in self.joint_seq]
            if missing_policy_joints:
                raise ValueError(f"Policy joints missing in metadata: {missing_policy_joints}")
            self.obs_joint_indices = np.array(
                [self.joint_seq.index(joint) for joint in self.policy_joint_names],
                dtype=np.int32,
            )
            self.mj2lab = np.array(
                [self.joint_xml.index(joint) for joint in self.joint_seq],
                dtype=np.int32,
            )
            ...
            if self.joint_pos_array_seq is None or self.action_scale is None:
                raise ValueError("Missing default_joint_pos or action_scale in ONNX metadata.")
            self.joint_pos_array = np.array(
                [self.joint_pos_array_seq[self.joint_seq.index(joint)] for joint in self.joint_xml],
                dtype=np.float32,
            )
            self.default_angles_lab = self.joint_pos_array_seq.astype(np.float32)
            self.action_scale_lab = self.action_scale.astype(np.float32)
            ...
            # Alignment phase settings (smoothly align to start frame pose)
            self.align_duration = config.get("align_duration", 0)  # seconds 1
            step = self.decimation_ * self.physical_dt
            self.align_steps = max(1, int(self.align_duration / step))
            self.align_counter = 0
            self.align_target_q = None
            self.in_align_phase = False
    ...
    def _clip_motion_frame(self, frame_idx: int) -> int:
        frame_idx = max(0, int(frame_idx))
        if self.motion_length is not None and self.motion_length > 0:
            frame_idx = min(frame_idx, self.motion_length - 1)
        return frame_idx

    def _get_motion_source_position(self, counter_step: int) -> float:
        if self.fast_play_source_frames > 0 and self.fast_play_output_steps > 0:
            if counter_step < self.fast_play_output_steps:
                step_span = self.fast_play_source_frames / float(self.fast_play_output_steps)
                return self.start_frame + (counter_step + 0.5) * step_span - 0.5
            return float(self.start_frame + self.fast_play_source_frames + (counter_step - self.fast_play_output_steps))
        return float(self.start_frame + counter_step)

    def _run_policy_outputs_at_frame(self, obs_tensor: np.ndarray, frame_idx: int):
        clipped_frame = self._clip_motion_frame(frame_idx)
        observation = {
            self.input_name[0]: obs_tensor,
            self.input_name[1]: np.array([[clipped_frame]], dtype=np.float32),
        }
        return self.ort_session.run(None, observation)

    def _blend_policy_outputs(self, outputs_a, outputs_b, alpha: float):
        blended_outputs = []
        for output_idx, (value_a, value_b) in enumerate(zip(outputs_a, outputs_b)):
            arr_a = np.asarray(value_a, dtype=np.float32)
            arr_b = np.asarray(value_b, dtype=np.float32)
            blended = (1.0 - alpha) * arr_a + alpha * arr_b
            if output_idx == 4:
                quat_norm = np.linalg.norm(blended, axis=-1, keepdims=True)
                blended = blended / np.where(quat_norm > 0, quat_norm, 1.0)
            blended_outputs.append(blended.astype(np.float32))
        return tuple(blended_outputs)

    def _get_policy_outputs(self, obs_tensor: np.ndarray, source_position: float):
        if self.fast_play_source_frames > 0 and self.fast_play_output_steps > 0 and self.counter_step < self.fast_play_output_steps:
            lower_frame = self._clip_motion_frame(int(np.floor(source_position)))
            upper_frame = self._clip_motion_frame(int(np.ceil(source_position)))
            alpha = float(source_position - np.floor(source_position))
            if upper_frame != lower_frame and alpha > 0.0:
                outputs_a = self._run_policy_outputs_at_frame(obs_tensor, lower_frame)
                outputs_b = self._run_policy_outputs_at_frame(obs_tensor, upper_frame)
                return self._blend_policy_outputs(outputs_a, outputs_b, alpha)
            return self._run_policy_outputs_at_frame(obs_tensor, lower_frame)
        return self._run_policy_outputs_at_frame(obs_tensor, int(round(source_position)))
    ...
    def _write_controlled_joint_commands(self, target_dof_pos_mj: np.ndarray):
        ...
        motor_slice = slice(self._command_base_idx, self._command_base_idx + self.motor_nums)
        # self.robot_data_.q_d_[motor_slice] = self.robot_data_.get_joint_pos()
        self.robot_data_.q_dot_d_[motor_slice] = 0.0
        self.robot_data_.tau_d_[motor_slice] = 0.0
    ...
    def on_enter(self):
        ...
        initial_time_index = self.start_frame
        if self.motion_length is not None and self.motion_length > 0:
            initial_time_index = min(initial_time_index, self.motion_length - 1)
        observation[self.input_name[1]] = np.array([[initial_time_index]], dtype=np.float32)
        ...
        # self.action = np.zeros(self.num_actions)
        # === Preload reference pose at start_frame for smooth alignment ===
        obs_zero = np.zeros((1, self.num_obs), dtype=np.float32)
        frame = self.start_frame
        if self.motion_length is not None and self.motion_length > 0:
            frame = min(frame, self.motion_length - 1)
        outputs = self.ort_session.run(
            None,
            {
                self.input_name[0]: obs_zero,
                self.input_name[1]: np.array([[frame]], dtype=np.float32),
            },
        )
        target_lab = outputs[1].squeeze(0)
        target_mj_23 = self.robot_data_.get_joint_pos()[self.locked_joint_map].astype(np.float32)
        target_mj_23[self.mj2lab] = target_lab
        target_mj = np.zeros(self.motor_nums)
        target_mj[self.locked_joint_map] = target_mj_23

        self.align_target_q = target_mj.copy()
        self.align_counter = 0
        self.in_align_phase = True
    ...
    def inner_run(self,flag:FSMControlFlag):    
        # ================= ALIGN PHASE =================
        if self.in_align_phase:
            q_now = self.robot_data_.get_joint_pos()

            alpha = min(1.0, self.align_counter / max(1, self.align_steps))
            smooth_alpha = alpha * alpha * (3 - 2 * alpha)  # smoothstep

            q_cmd = (1.0 - smooth_alpha) * q_now + smooth_alpha * self.align_target_q

            self._write_controlled_joint_commands(q_cmd)

            self.align_counter += 1
            if self.align_counter >= self.align_steps:
                self.in_align_phase = False
        ...
        hold_start = 500#500#800#400
        resume_frame = 1400#3800
        hold_cap_frame = 1900000
        base_time_index = self._get_motion_source_position(self.counter_step)
        time_index = base_time_index
        if base_time_index >= hold_start:
            ...
            if not self.resume_from_hold:
                ...
                time_index = resume_frame + (base_time_index - (hold_start + self.hold_duration))

        if time_index >= hold_cap_frame:
            time_index = hold_cap_frame
        ...
        outputs_result = self._get_policy_outputs(mimic_obs_tensor, time_index)
        (
            self.action,
            self.ref_joint_pos,
            self.ref_joint_vel,
        ...
        target_dof_pos_mj_23dof = self.robot_data_.get_joint_pos()[self.locked_joint_map].astype(np.float32)
        ...
        target_dof_pos_mj_23dof[self.mj2lab] = blended_target
    ...
    def run(self, flag: FSMControlFlag):
        ...
        self.set_kp_kd()
    def set_kp_kd(self):
        # self.robot_data_.joint_kp_p_[:self.motor_nums] = 0.0
        # self.robot_data_.joint_kd_p_[:self.motor_nums] = 0.0
