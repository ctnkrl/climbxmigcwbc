...
class FSMStatePUSHUP(FSMState):
    def __init__(self, robot_data: RobotData, config_path: Optional[str] = None, variant_name: str = "default"):
        ...
        config_path = os.path.join(current_dir, "config", "pushup.yaml")
        ...
        with open(self.config_path, "r") as f:
            ...
            loop_config = config.get("motion_loop", {}) or {}
            self.motion_loop_enabled = bool(loop_config.get("enabled", True))
            self.motion_loop_start_frame = self._optional_int(loop_config.get("start_frame", 60))
            self.motion_loop_end_frame = self._optional_int(loop_config.get("end_frame", 100))
            self.motion_loop_default_length = max(1, int(loop_config.get("default_length", 40)))
            self.motion_loop_post_frames = max(0, int(loop_config.get("post_loop_frames", 40)))
            ...
            self.ort_session = onnxruntime.InferenceSession(self.onnx_path, sess_options=options, providers=['CPUExecutionProvider'])
            ...
            for prop in metadata_props:
                ...
                if prop.key in ("motion_length", "time_step_total"):
                    try:
                        ...
                        logger.warning(f"[FSMStateBeyondMimic] Invalid motion_length metadata value: {prop.value}")

            if self.motion_length is not None:
                try:
                    self.motion_length = int(self.motion_length)
                except (TypeError, ValueError):
                    logger.warning(f"[FSMStateBeyondMimic] Invalid motion_length config value: {self.motion_length}")
            ...
            self.joint_xml = [
                ...
                "elbow_pitch_l_joint", "wrist_pitch_l_joint",
                "shoulder_pitch_r_joint", "shoulder_roll_r_joint", "shoulder_yaw_r_joint",
                "elbow_pitch_r_joint", "wrist_pitch_r_joint",
            ]
            # 从MjXUML顺序映射到实验室顺序
            ...
            self.resume_from_loop = False
            self._loop_resume_step: Optional[int] = None
            self._loop_resume_remaining = 0
    ...
    def _play_remote_audio_YYW(self):
        """Executes the audio playback command on the remote machine via SSH with sshpass."""
        host = "192.168.41.2"
        username = "nvidia"
        password = "nvidia"  # Exposed in process list; avoid in production.
        command = "aplay ~/Documents/123456wenge.wav"
        
        try:
            full_command = [
                "sshpass", "-p", password,
                "ssh", "-o", "StrictHostKeyChecking=no",  # Auto-accept host key
                f"{username}@{host}", command
            ]
            result = subprocess.run(full_command, capture_output=True, text=True)
            if result.returncode != 0:
                logger.warning(f"Remote command error: {result.stderr.strip()}")
            else:
                logger.info("Remote audio playback initiated successfully.")
        except Exception as e:
            logger.warning(f"SSH command execution failed: {e}")
    ...
    def on_enter(self):
        ...
        #     threading.Thread(target=self._play_remote_audio_YYW).start()
        
        # threading.Timer(delay_seconds, delayed_playback).start()
        ...
        self._warmup_inference_counter = 0
        self.resume_from_loop = False
        self._loop_resume_step = None
        self._loop_resume_remaining = 0
        logger.info(f"[FSMStatePUSHUP], config={self.config_path}")
    ...
    def inner_run(self, flag: FSMControlFlag):
        ...
        base_time_index = max(self.counter_step - self.warm_start_steps, 0)
        time_index = self._get_motion_time_index(base_time_index, flag)
        ...
        target_dof_pos_mj_23dof = np.zeros(25)
    ...
    def _resolve_motion_loop_bounds(self) -> Optional[tuple[int, int]]:
        if not self.motion_loop_enabled:
            return None

        start_frame = self.motion_loop_start_frame
        end_frame = self.motion_loop_end_frame

        if (start_frame is None or end_frame is None) and self.motion_length is not None and self.motion_length > 0:
            last_loop_frame = max(0, self.motion_length - self.motion_loop_post_frames - 1)
            inferred_length = min(self.motion_loop_default_length, last_loop_frame + 1)
            inferred_start = max(0, last_loop_frame - inferred_length + 1)
            if start_frame is None:
                start_frame = inferred_start
            if end_frame is None:
                end_frame = last_loop_frame

        if start_frame is None or end_frame is None:
            logger.warning("[FSMStatePUSHUP] motion_loop enabled but loop bounds are unavailable.")
            return None

        start_frame = max(0, int(start_frame))
        end_frame = max(start_frame, int(end_frame))
        if self.motion_length is not None and self.motion_length > 0:
            end_frame = min(end_frame, self.motion_length - 1)

        if end_frame <= start_frame:
            logger.warning(
                f"[FSMStatePUSHUP] Invalid motion_loop bounds: start={start_frame}, end={end_frame}"
            )
            return None

        return start_frame, end_frame

    def _get_motion_time_index(self, base_time_index: int, flag: FSMControlFlag) -> int:
        loop_bounds = self._resolve_motion_loop_bounds()
        if loop_bounds is None:
            return base_time_index

        loop_start, loop_end = loop_bounds
        loop_length = loop_end - loop_start + 1

        if flag.fsm_resume_command == "pushup_resume":
            self.resume_from_loop = True

        if base_time_index < loop_start:
            return base_time_index

        if self.resume_from_loop and self._loop_resume_step is None:
            self._loop_resume_step = base_time_index
            cycle_pos = (base_time_index - loop_start) % loop_length
            self._loop_resume_remaining = loop_length - cycle_pos

        if not self.resume_from_loop:
            return loop_start + ((base_time_index - loop_start) % loop_length)

        progress_after_resume = base_time_index - self._loop_resume_step
        if progress_after_resume < self._loop_resume_remaining:
            return loop_start + ((base_time_index - loop_start) % loop_length)

        time_index = loop_end + 1 + (progress_after_resume - self._loop_resume_remaining)
        if self.motion_length is not None and self.motion_length > 0:
            time_index = min(time_index, self.motion_length - 1)
        return time_index
    ...
    def _get_current_joint_pos_lab(self) -> np.ndarray:
        try:
            ...
            logger.warning(f"[FSMStateBeyondMimic] Failed to read current joint pose: {exc}")
    ...
    def _get_onnx_first_frame_lab(self) -> np.ndarray:
        try:
            ...
            logger.warning(f"[FSMStateBeyondMimic] Failed to read ONNX first frame: {exc}")
            return np.array(self.default_angles_lab, copy=True)