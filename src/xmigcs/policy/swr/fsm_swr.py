...
class FSMStateSWR(FSMState):
    ...
    def __init__(self, robot_data: RobotData):
        ...
        config_path = os.path.join(current_dir, "config", "swr.yaml")
        ...
        self.run_lin_vel_x_threshold_ = float(command_config.get('run_lin_vel_x_threshold', 1.2))
        ...
        self.forward_joystick_slow_zone_ = float(command_config.get('forward_joystick_slow_zone', 0.9))
        self.forward_joystick_slow_zone_ = float(np.clip(self.forward_joystick_slow_zone_, 1e-3, 0.999))
        self.forward_joystick_slow_speed_ = float(command_config.get('forward_joystick_slow_speed', 1.0))
        self.forward_high_speed_accel_rate_ = float(command_config.get('forward_high_speed_accel_rate', 0.6))
        self.forward_high_speed_decel_rate_ = float(command_config.get('forward_high_speed_decel_rate', 4.0))
    ...
    def run(self, flag: FSMControlFlag):
        ...
        if self.robot_data_.control_step_ % self.decimation_ == 0:
            ...
            commanded_pos = commanded_pos[:len(self.joint_xml)]
        ...
        self.robot_data_.joint_kp_p_[:len(self.joint_xml
    ...
    def compute_observation(self, flag: FSMControlFlag):
        ...
        # target_command, _ = self._project_command_to_xwalk_modes_numpy(raw_command)
        # command, _ = self._smooth_policy_command(target_command)
        command = raw_command
    ...
    def _smooth_policy_command(self, target_command: np.ndarray) -> tuple[np.ndarray, str]:
        ...
        next_command = alpha * target_command + (1.0 - alpha) * self.smoothed_command_
        next_command[0] = self._rate_limit_forward_command(
            current=float(self.smoothed_command_[0]),
            target=float(target_command[0]),
            filtered=float(next_command[0]),
        )
        self.smoothed_command_[:] = next_command
    ...
    def _remap_forward_joystick_command(self, raw_command: np.ndarray) -> np.ndarray:
        """Use most forward-stick travel for low-speed control, reserving the tip for run."""
        command = raw_command.copy()
        if command.shape[0] < 1 or command[0] <= 0.0:
            return command

        _, lin_x_high = self.command_lin_vel_x_range_
        max_forward_speed = max(float(lin_x_high), self.command_zero_epsilon_)
        joystick_ratio = float(np.clip(command[0] / max_forward_speed, 0.0, 1.0))

        if joystick_ratio <= self.forward_joystick_slow_zone_:
            command[0] = (
                joystick_ratio / self.forward_joystick_slow_zone_
            ) * self.forward_joystick_slow_speed_
        else:
            fast_ratio = (joystick_ratio - self.forward_joystick_slow_zone_) / (
                1.0 - self.forward_joystick_slow_zone_
            )
            command[0] = self.forward_joystick_slow_speed_ + fast_ratio * (
                max_forward_speed - self.forward_joystick_slow_speed_
            )

        return command

    def _rate_limit_forward_command(self, current: float, target: float, filtered: float) -> float:
        if self.inference_period_s_ <= 0.0:
            return filtered

        high_speed_floor = self.forward_joystick_slow_speed_
        if target > current and target > high_speed_floor:
            max_step = max(self.forward_high_speed_accel_rate_, 0.0) * self.inference_period_s_
            if current < high_speed_floor:
                return min(filtered, max(high_speed_floor, current + max_step))
            return min(filtered, current + max_step)

        if target < current and current > 0.0:
            max_step = max(self.forward_high_speed_decel_rate_, 0.0) * self.inference_period_s_
            return max(filtered, current - max_step)
    ...
    def _compute_amp_condition_from_command(self, command: np.ndarray) -> np.ndarray:
        ...
        if vx >= self.run_lin_vel_x_threshold_:
            amp_condition[5] = 1.0
            return amp_condition
